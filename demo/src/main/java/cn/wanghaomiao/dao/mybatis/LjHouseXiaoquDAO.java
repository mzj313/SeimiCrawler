...
public interface LjHouseXiaoquDAO {
	...
	@Insert("insert into ljhouse_xiaoqu (title,averageprice,houseInfo,positionInfo,positionInfo1,positionInfo2,tagList,num,url,update_time) "
			+ "values (#{lj.title},#{lj.averagePrice},#{lj.houseInfo},#{lj.positionInfo},#{lj.positionInfo1},#{lj.positionInfo2},#{lj.tagList},#{lj.num},#{lj.url},now())")
