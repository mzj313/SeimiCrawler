package org.mzj.publicaccounts;
...
public class WxAccountCrawler extends BreadthCrawler {
    ...
    public static final Logger logger = LoggerFactory.getLogger(WxAccountCrawler.class);
    ...
    public WxAccountCrawler(String crawlPath, String historyKeysPath) throws Exception {
        ...
        logger.info("initializing history-keys-filter ......");
        this.setNextFilter(new HistoryKeysFilter(historyKeysPath));
        logger.info("creating history-keys-writer");
    ...
    public void visit(Page page, CrawlDatums next) {
        ...
        if (page.matchType("account_search")) {
            ...
            if (accountLinkEle == null) {
                logger.info("公众号\"" + account + "\"不存在，请给出准确的公众号名");
                return;
            }
            //防止公众号名错误
            String detectedAccount = accountLinkEle.text().trim();
            if (!account.equals(detectedAccount)) {
                logger.info("公众号\"" + account + "\"与搜索结果\"" + detectedAccount + "\"名称不符，请给出准确的公众号名");
                return;
            }
            String accountUrl = accountLinkEle.attr("abs:href").replace("&amp;", "&");
            logger.info("详情页accountUrl: " + accountUrl);
            ...
            try {
                ...
                logger.info("writer exception", ex);
            }
